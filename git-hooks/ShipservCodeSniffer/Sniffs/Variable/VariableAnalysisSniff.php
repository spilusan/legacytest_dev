...
/**
 * Modification from ShipServ (Claudio): change all addWarnings to addFixableError
 */
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class ShipservCodeSniffer_Sniffs_Variable_VariableAnalysisSniff implements PHP_CodeSniffer_Sniff
{
    /**
    ...
    function markVariableDeclaration($varName, $scopeType, $typeHint, $stackPtr, $currScope, $permitMatchingRedeclaration = false) {
        ...
        if (isset($varInfo->scopeType)) {
            if (($permitMatchingRedeclaration === false) ||
                ...
                $this->currentFile->addFixableError(
    ...
    function markVariableReadAndWarnIfUndefined($phpcsFile, $varName, $stackPtr, $currScope) {
        ...
        if ($this->isVariableUndefined($varName, $stackPtr, $currScope) === true && !(substr($phpcsFile->getFilename(), -6) === '.phtml' && $varName === 'this')) {
            // We haven't been defined by this point.
            $phpcsFile->addFixableError("Variable %s is undefined.", $stackPtr,
                'UndefinedVariable',
                array("\${$varName}"));
        }
        return true;
