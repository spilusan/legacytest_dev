 ...
 */
class Myshipserv_EmailCampaign_SIRInvitationPremiumListing_Generator {
	...
	public function generate()
	{
		...
		echo "Start sending SIR invite reminder to Premium Lister that have more than 5 enquiries in past year\n-------------------------------------------------------------------------------------------------------------------------------------\n\n";
		...
		foreach( $rows  as $row )
		{
			...
			echo "-- ". $job . ". sending an email to: " . $email . " (" . ( ( $isTrusted == 1 || $userType == "PAGES" || $userType == "ENQUIRY") ? "TRUSTED - " : "NOT TRUSTED - ") . $userType . " user)\n";
			...
			try 
			{
				...
				{
					echo "---- error, this supplier is not published: " . $row['PMR_TNID'] . "\n";
			...
			if( $error == false )
			{
				...
				try
				{
					...
					$notificationManager->sendSIRInvitePremiumListing( $email, $subject, $supplier, $report, $yearAgoAsDate->format("d F Y"), $userType, $userId, $this->getDb(), $this->mode, $row);
					$adapter->setTimestamp( "error", $email, self::CAMPAIGN_ID );
