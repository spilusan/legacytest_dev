 ...
 */
class Myshipserv_View_Helper_Requirejs extends Zend_View_Helper_Abstract
{
	private static $modules = array();
	private static $definitions = array();
    private static $paths = array(
        /*css path is added during the render() method*/
        'jquery'        => 'backbone/lib/jquery.loader',
	...
	public function render() {
		...
		if (count(self::$modules) > 0) {
			...
			$modulesPath = '/js/' . Myshipserv_Config::getCachebusterTagAddition() . 'modules/';
			...
			if (($config['shipserv']['cdn']['use'] == 1)) 
			{
				...
				$cssPath =  str_replace('shipserv.net','shipserv.com',str_replace( 'http://', 'https://', $cssPath));
				$modulesPath =  str_replace('shipserv.net','shipserv.com',str_replace( 'http://', 'https://', $modulesPath));				
	        }
			...
			$headscript->prependFile('/js/modules/backbone/lib/jquery.min.1.8.3.js', 'text/javascript');
