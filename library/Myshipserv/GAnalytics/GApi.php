 ...
 *
 * NOTE:
 * Class names modified by ajwp
 * Some code also patched (commented 'ajwp')
 */
class Myshipserv_GAnalytics_GApi
{
  ...
  {
    $response = $this->httpRequest(Myshipserv_GAnalytics_GApi::account_data_url, array('start-index'=>$start_index,'max-results'=>$max_results), null, $this->generateAuthHeader());
  ...
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=30)
  {
	...
	if(is_array($dimensions))
    {
	  ...
	  // ajwp: added 'if' clause to fix bug with empty dimensions array
	  if (strlen($dimensions_string) >= 1)
	  {
    ...
    $parameters['prettyprint'] = Myshipserv_GAnalytics_GApi::dev_mode ? 'true' : 'false';
    
    $response = $this->httpRequest(Myshipserv_GAnalytics_GApi::report_data_url, $parameters, null, $this->generateAuthHeader());
   ...
   * @return Array of Myshipserv_GAnalytics_GApiAccountEntry objects
   */
  protected function accountObjectMapper($xml_string)
  {
    ...
    foreach($xml->entry as $entry)
    {
      ...
      $results[] = new Myshipserv_GAnalytics_GApiAccountEntry($properties);
    }
   ...
   *
   * @param String $xml_string
   * @return Array of Myshipserv_GAnalytics_GApiReportEntry objects
   */
  protected function reportObjectMapper($xml_string)
  {
    ...
    foreach($xml->entry as $entry)
    {
      ...
      $results[] = new Myshipserv_GAnalytics_GApiReportEntry($metrics,$dimensions);
    }
  ...
  protected function authenticateUser($email, $password)
  {
    $post_variables = array(
      ...
      'Email' => $email,
      'Passwd' => $password,
      'source' => Myshipserv_GAnalytics_GApi::interface_name,
      'service' => 'analytics'
    );
    
    $response = $this->httpRequest(Myshipserv_GAnalytics_GApi::client_login_url,null,$post_variables);
  ...
  {
    $interface = Myshipserv_GAnalytics_GApi::http_interface;
    
    if(Myshipserv_GAnalytics_GApi::http_interface =='auto')
    {
    ...
    {
      throw new Exception('Invalid http interface defined. No such interface "' . Myshipserv_GAnalytics_GApi::http_interface . '"');
    }
  }
  ...
  public function __call($name,$parameters)
  {
    ...
    $parameter_key = Myshipserv_GAnalytics_GApi::array_key_exists_nc($name,$this->report_root_parameters);
    ...
    $aggregate_metric_key = Myshipserv_GAnalytics_GApi::array_key_exists_nc($name,$this->report_aggregate_metrics);
 ...
 *
 */
class Myshipserv_GAnalytics_GApiAccountEntry
{
  ...
  public function __call($name,$parameters)
  {
    ...
    $property_key = Myshipserv_GAnalytics_GApi::array_key_exists_nc($name,$this->properties);
 ...
 *
 */
class Myshipserv_GAnalytics_GApiReportEntry
{
  ...
  public function __call($name,$parameters)
  {
    ...
    $metric_key = Myshipserv_GAnalytics_GApi::array_key_exists_nc($name,$this->metrics);
    ...
    $dimension_key = Myshipserv_GAnalytics_GApi::array_key_exists_nc($name,$this->dimensions);
