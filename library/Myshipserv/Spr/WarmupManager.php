 ...
 * 
 * This file is deprecated now (05/10/2017)
 * @author attilaolbrich
 *
 */

class Myshipserv_Spr_WarmupManager
{
    /**
	...
	protected function __construct()
	{
	    ...
	    $this->reportService =  Myshipserv_ReportService_Gateway::getInstance(Myshipserv_ReportService_Gateway::REPORT_SPR);
	}
	...
	protected function warmCache($scriptName, $groupName, $serviceName, $rewriteCache = true)
	{
		...
		switch ($groupName) {
            case 'shipserv-average':
                ...
                $memcacheTtl =  Myshipserv_ReportService_Gateway::INFINITE_MEMCACHE_TTL;
                break;
		...
		foreach (self::$periodList as $curPeriod) {
            ...
            // add two more calculation with parameter variations to cache all possible cache

            // no value for period parameter - defaults to 'breakdown by month'
            $sessionParams['period'] = null;
            $sessions[] = $sessionParams;

            // no period parameter - also defaults to 'breakdown by month'
            unset($sessionParams['period']);
            $sessions[] = $sessionParams;
		}
