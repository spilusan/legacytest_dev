<?php
class Shipserv_Report_Supplier_Gmv extends Shipserv_Report
{	
...
gmv_data AS(
  SELECT
    ...
    , MAX(rep.ord_total_cost_discounted_usd) KEEP (DENSE_RANK FIRST ORDER BY rep.primary_id DESC)
        OVER (PARTITION BY rep.ord_internal_ref_no, rep.ord_original_no) total_cost_usd    
  FROM
    ord o
    , poc p
    , billable_po rep        
  ...
  WHERE
    ...
    AND o.ord_submitted_date BETWEEN TO_DATE(:startDate, 'DD-MON-YYYY') AND TO_DATE(:endDate, 'DD-MON-YYYY')+.99999
    AND o.ord_internal_ref_no=p.ord_internal_ref_no (+)
    AND o.ord_internal_ref_no = rep.ord_original_no(+)
...
match_data AS (
  SELECT
    ...
    , p.ord_internal_ref_no poc_internal_ref_no
    , r.original_date
	, (" . $this->getGroupingString('r.original_date') . ") grouping_string   		
    , s.spb_branch_code
	, s.parent_branch_code
    , MAX(rep.ord_total_cost_discounted_usd) KEEP (DENSE_RANK FIRST ORDER BY rep.primary_id DESC)
        OVER (PARTITION BY rep.ord_internal_ref_no, rep.ord_original_no) total_cost_usd
  FROM
	...
	, supplier_data s
  WHERE
    r.rfq_submitted_date BETWEEN TO_DATE(:startDate, 'DD-MON-YYYY') AND TO_DATE(:endDate, 'DD-MON-YYYY')+.99999
...
tradenet_data AS (
  ...
  WHERE
    ...
    AND r.rfq_submitted_date BETWEEN TO_DATE(:startDate, 'DD-MON-YYYY') AND TO_DATE(:endDate, 'DD-MON-YYYY')+.99999
...
(
  SELECT 
    ...
    , SUM(CASE WHEN m.qot_internal_ref_no IS NOT null THEN 1 ELSE 0 END) m_total_qot
    , SUM(CASE WHEN m.ord_internal_ref_no IS NOT null THEN 1 ELSE 0 END) m_total_ord
    , SUM(CASE WHEN m.poc_internal_ref_no IS NOT null THEN 1 ELSE 0 END) m_total_poc
...
(
  SELECT 
    ...
    , CASE WHEN COUNT(DISTINCT tn.qot_internal_ref_no) = 0 THEN 0 ELSE COUNT(DISTINCT tn.qot_internal_ref_no) -1 END tn_total_qot
	, CASE WHEN COUNT(DISTINCT tn.ord_internal_ref_no) = 0 THEN 0 ELSE COUNT(DISTINCT tn.ord_internal_ref_no) -1 END tn_total_ord
    , CASE WHEN COUNT(DISTINCT tn.poc_internal_ref_no) = 0 THEN 0 ELSE COUNT(DISTINCT tn.poc_internal_ref_no) -1 END tn_total_poc
...
(
  SELECT 
    ...
    , SUM(CASE WHEN pgs.qot_internal_ref_no IS NOT null THEN 1 ELSE 0 END) pgs_total_qot
    , SUM(CASE WHEN pgs.ord_internal_ref_no IS NOT null THEN 1 ELSE 0 END) pgs_total_ord
    , SUM(CASE WHEN pgs.poc_internal_ref_no IS NOT null THEN 1 ELSE 0 END) pgs_total_poc
...
SELECT
  ...
  , AVG(NVL(tn_quote_rate, 0)) tn_quote_rate
  , AVG(NVL(tn_win_rate, 0)) tn_win_rate
  ...
  , AVG(NVL(m_quote_rate, 0)) m_quote_rate
  , AVG(NVL(m_win_rate, 0)) m_win_rate
  ...
  , AVG(NVL(ovr_quote_rate, 0)) ovr_quote_rate
  , AVG(NVL(ovr_win_rate, 0)) ovr_win_rate
...
ORDER BY 
  " . ( ( $rolledUp === true ) ? "parent_branch_code":"spb_branch_code" ) . ", grouping_string ASC
		...
		$key = "Shipserv_Report_Supplier_Gmv" . md5($sql) . print_r($params, true);
