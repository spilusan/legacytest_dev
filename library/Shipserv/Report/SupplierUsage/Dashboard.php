...
*/
class Shipserv_Report_SupplierUsage_Dashboard
{
	const REPORT_ROOT = 0;
	...
	* @param boolean $hasLInit 
	* @param bool $extended Full text return for engagement level
	* @return array Array of result row objects
	*/
	public function getResponse($hasLimit = true, $extended = false)
	{
		$resultProcessorObj = null;
		
		switch ($this->reportType) {
			case self::REPORT_ROOT:
				$resultProcessorObj = new Shipserv_Report_SupplierUsage_Rec_Root($this->params, $extended);
				break;
		...
		return $resultProcessorObj->getData($hasLimit);
