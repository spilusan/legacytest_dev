<?php
/**
 * Handle the creation of enquiry worksheet
 * for the documentation please refer to TargetedSearch.php class located on the same folder
 * @author Elvir <eleonard@shipserv.com>
 */

class Shipserv_Report_Excel_EnquiryStatistic extends Shipserv_Report_Excel
{
	...
	public function create()
	{
		...
		$objWorksheet->getTabColor()->setRGB( $this->colors["worksheets"]["Enquiries"] );
		...
		$this->objPHPExcel->getActiveSheet()->getStyle("A2")->applyFromArray(
			array( 
				...
				, "fill" => array( 
					...
					, "color" => array( "argb" => $this->colors["worksheets"]["Enquiries"] )
				)
			)
		);
		$this->objPHPExcel->getActiveSheet()->setCellValue('A2', "Pages RFQ statistics");
		...
		//$startRow = $this->createBrandAndCategorySearches( $startRow );		
	}
	...
	private function createTableGroupByUserType( $startRow )
	{
		...
		$this->write( array('A', $row+1), null, $style["bold"], "Types of user that send RFQ to this supplier" );
		...
		$intCol = 0;
		$data = $this->data[0];
		foreach( (array) $data["enquiry-summary"]["enquiries-sent-by-user-type"] as $row )
		{
	...
	private function createTopSearchesLeadToImpressionTableGroupByKeyword( $startRow )
	{
		...
		// header for top brands
		$this->write( array('A', $row+1), null, $style["bold"], "Profile Page views" );
		...
		$intCol = 0;
		$data = $this->data[0];
		foreach( (array) $data["enquiry-summary"]["enquiries-sent-by-search-keywords"] as $row )
		{
			$xGrid[$this->getNextColumn('A', $intCol)][] = $row['name'];
			$xGrid[$this->getNextColumn('B', $intCol)][] = intval($row['count']);
		}
