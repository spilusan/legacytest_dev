define([
    ...
    'jqueryui/datepicker',
	'../views/selectorView',
	'text!templates/reports/sir2/tpl/filters.html',
	'text!templates/reports/sir2/tpl/products.html'
], function(
	$,
	_, 
	Backbone,
	...
	var filtersView = Backbone.View.extend({
		...
		render: function() {
			...
			$('body').delegate('.periodSel', 'click', function(e){
				thisView.togglePeriodDetails(e);
			});
			$('body').delegate('.periodComp', 'click', function(e){
				thisView.toggleCompareTo(e);
			});
			$('body').delegate('.marketComp', 'click', function(e){
				thisView.toggleChecker(e);
			});
			$('body').delegate('input[name="apply"]', 'click', function(e){
				e.preventDefault();
			...
			$('body').delegate('input.date', 'change', function(e){
				thisView.addSlashes(e);
				if($(e.target).val() != ""){
				}
			});
			$('body').delegate('.quickSel', 'change', function(e){
		...
		quickSelect: function(e){
			...
			else if($(e.target).val() == 30){
				from = from.setMonth(from.getMonth() - 1);
			}
			else if($(e.target).val() == 90){
				from = from.setMonth(from.getMonth() - 3); 
			}
			else if($(e.target).val() == 365){
