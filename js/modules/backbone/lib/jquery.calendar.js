...
(function ($) {
	...
	function initCalendar(wrapper, options) {
		...
		headerLabel.bind('click', function () { 
			...
			currentCalendar.data('currentDay', new Date().getDate());
		...
		currentCalendar = wrapper;
	
		var startDate = (options.startDate) ?  options.startDate : new Date();
		currentCalendar.data('currentYear', startDate.getFullYear());
		currentCalendar.data('currentMonth', startDate.getMonth());
		currentCalendar.data('currentDay', startDate.getDate());
		selectMonth(null, options, currentCalendar.data('currentMonth'), currentCalendar.data('currentYear'));

		calendarFrame.find('td').each(function(){
			if (parseInt($(this).html()) === currentCalendar.currentDay) {
	...
	function selectMonth(next, options, month, year) {
		...
		currentCalendar.data('currentMonth', month);
		...
		currentCalendar.data(
			'currentYear',
			year || ((next && currentCalendar.data('currentMonth') === 0) ? tmpYear + 1 : (!next && currentCalendar.data('currentMonth') === 11) ? tmpYear - 1 : tmpYear)
		);
		var calendar = createCalendar(currentCalendar.data('currentMonth'), currentCalendar.data('currentYear'), options), frame = calendar.frame();
		...
		frame.on('click', 'td', function () {
			...
			$(this).addClass('selected');
			currentCalendar = $(this).parents('.calendar-frame').parent();
			currentCalendar.data('currentDay', $(this).text());
			triggerSelectEvent(options.onSelect);
		});
	}	
	...
	function createCalendar(month, year, options) {
		...
		if(currentCalendar.data('currentMonth') === new Date().getMonth()) { 
	...
	function triggerSelectEvent(event) {
		var date = new Date(currentCalendar.data('currentYear'), currentCalendar.data('currentMonth'), currentCalendar.data('currentDay'));
