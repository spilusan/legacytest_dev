define([
	'jquery',
	'underscore',
	'Backbone',
	'handlebars',
	'backbone/shared/hbh/general',
	'libs/jquery.tools.overlay.modified',
	'text!templates/profile/targetCustomers/tpl/targetedRow.html'
], function(
	$, 
	_, 
	Backbone,
	Hb,
	HbhGen,
	Modal,
	targetedRowTpl
){
	var targetedRowView = Backbone.View.extend({
		tagName: 'tr',

		template: Handlebars.compile(targetedRowTpl),

		events: {
			'click input[name="exclude"]' : 'showExcludePopup',
			'click input[name="doExclude"]' : 'exclude',
			'mouseover img.locked' : 'showLockedTooltip',
            'mouseout img.locked' : 'hideLockedTooltip'
		},
		
		initialize: function(){
			_.bindAll(this, 'render');
			this.model.view = this;
		},

	    render: function() {
			var data = this.model.attributes;

			if(data.OrderValue === null){
				data.OrderValue = 0;
			}

			data.rate.rateTarget = data.rate.rateTarget.toFixed(2);

			data.vessels = [];
			var vesselTypeCount = data.vessel.vesselTypeList.length;
			var count = 0;

			_.each(data.vessel.vesselTypeList, function(item){
				count++;
				if(count <= 3){
					if(count != vesselTypeCount && count < 3){
						item = item + ", ";
					}
					data.vessels.push(item);
				}
				else if(count === 4){
					if(vesselTypeCount === 4){
						var typeCopy = "type";
					}
					else {
						var typeCopy = "types";
					}
					data.vessels.push('and ' + (vesselTypeCount - 3) + ' other vessel ' + typeCopy);
				}
			}, this);

			if(data.rfqsReceived === null){
				data.rfqsReceived = 0;
			}

			var html = this.template(data);

			$(this.el).html(html);

	        return this;
	    },

	    showLockedTooltip: function(){
            $(this.el).find('.lockTooltip').show();
        },

        hideLockedTooltip: function(){
            $(this.el).find('.lockTooltip').hide();
        },

	    showExcludePopup: function(e){
	    	var thisView = this;
	    	$('.excludePopup .custName').html(this.model.attributes.name);
	    	this.openXDialog();
	    	this.doExcludeButton = $(e.target).next('input[name="doExclude"]');
	    	$('input[name="xclude"]').unbind().bind('click', function(e){
	    		$(thisView.doExcludeButton).click();
	    	});
	    },

	    exclude: function(e){
	    	e.preventDefault();
	    	$('#modalX.excludePopup').overlay().close();
	    	var thisView = this;

	    	$.ajax({
				method: "GET",
				url: "/profile/target-customers-request",
				data: { 
					type: "exclude",
					buyerId: thisView.model.attributes.buyerId
				}
			})
			.done(function(msg) {
				thisView.deleteRow();
				thisView.parent.getExcluded();
			})
			.fail(function(msg){
				alert('An error occurred.');
			});
	    },

	    cancelModal: function() {
	    	$('#modalX.excludePopup').overlay().close();
	    },

	    deleteRow: function(){
	    	this.model.destroy();
	    	this.remove();
	    },

	    openXDialog: function() {
	    	$("#modalX").overlay({
		        mask: 'black',
		        left: 'center',
		        fixed: 'true',
		 
		        onBeforeLoad: function() {
		            var windowWidth = $(window).width();
		        	var modalWidth = $('#modalX').width();
		        	var posLeft = windowWidth/2 - modalWidth/2;

		        	$('#modalX').css('left', posLeft);
		        },

		        onLoad: function() {
		        	$(window).resize(function(){
		        		var windowWidth = $(window).width();
		        		var modalWidth = $('#modalX').width();
		        		var posLeft = windowWidth/2 - modalWidth/2;

		        		$('#modalX').css('left', posLeft);
		        	});
		        }
			});

			$('#modalX.excludePopup').overlay().load();
	    }
	});

	return targetedRowView;
});