...
qq.FileUploader.prototype = {
    ...
    _addToList: function(id, fileName){
		...
		// ds - adds a unique id to the list item
		qq.setId(item, 'item-' + id);
	...
	_bindRemoveEvent: function(){
        ...
        qq.attach(list, 'click', function(e){
            ...
            if (qq.hasClass(target, self._classes.remove)){
				...
				// remove the file command field
				$(self._options.fileControlPrefix + tmp[1]).remove();
				// check if the no attachments header should be shown:
				if ($('#upload-list').children().length == 0 && $('#no-attachments').is(':hidden')) {
					$('#no-attachments').slideDown();
				}
            }
        });
    }
